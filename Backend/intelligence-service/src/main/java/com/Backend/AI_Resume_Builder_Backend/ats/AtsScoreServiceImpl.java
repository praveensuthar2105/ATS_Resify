...
import com.Backend.AI_Resume_Builder_Backend.common.PromptUtils;
...
public class AtsScoreServiceImpl implements AtsScoreService {
    ...
    public AtsScoreServiceImpl(GeminiService geminiService,
    ...
    public Map<String, Object> getAtsScore(MultipartFile resumeFile, String jobDescription) throws IOException {
        ...
        String promptTemplate = PromptUtils.loadPromptFromFile("ats_prompt.txt");
        ...
        String prompt = PromptUtils.putValueToTemplate(promptTemplate, values);
        ...
        for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                result = PromptUtils.parseMultipleResponses(response);
    ...
    @Override
    public Map<String, Object> getAtsScoreFromText(String resumeText, String jobDescription) throws IOException {
        log.info("Processing ATS score from pre-extracted text (length: {} chars)",
                resumeText != null ? resumeText.length() : 0);

        // Build pre-analysis from the text directly (skip PDF extraction)
        String preAnalysis = buildPreAnalysis(resumeText, jobDescription);
        ...
        Map<String, String> values = new HashMap<>();
        values.put("resumeText", resumeText);
        values.put("formatHints", "Pre-extracted text (async processing)");
        values.put("preAnalysis", preAnalysis);
        ...
        // Retry up to MAX_RETRIES times with exponential backoff (same as getAtsScore)
        Map<String, Object> result = null;
        for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                Optional<String> responseOpt = geminiService.generateContent(prompt);
                if (responseOpt.isEmpty()) {
                    log.warn("ATS text analysis attempt {} returned empty response, retrying...", attempt);
                    sleepWithBackoff(attempt);
                    continue;
                }
                ...
                List<String> missingKeys = getMissingKeys(result);
                log.warn("ATS text response missing keys {} on attempt {}, retrying...", missingKeys, attempt);

            } catch (Exception e) {
                log.warn("ATS text analysis attempt {} failed: {}", attempt, e.getMessage());
            }
        ...
        if (result != null) {
            List<String> missingKeys = getMissingKeys(result);
            if (!missingKeys.isEmpty()) {
                log.warn("Returning partial ATS text result after {} attempts, missing keys: {}", MAX_RETRIES, missingKeys);
                result.put("partial", true);
                result.put("missingKeys", missingKeys);
            }
            result.put("resumeText", resumeText);
        ...
        throw new IOException("Failed to get valid ATS text analysis after " + MAX_RETRIES + " attempts");
    }
    ...
    private PdfExtractionResult extractFromPdf(MultipartFile file) throws IOException {
        ...
        try (InputStream is = file.getInputStream()) {
            ...
            if (is.read(header) < 4 || !new String(header, java.nio.charset.StandardCharsets.US_ASCII).equals("%PDF")) {
                throw new IllegalArgumentException("Invalid file format. Only valid PDF files are allowed.");
            }
        }
        ...
        try (PDDocument doc = PDDocument.load(pdfBytes)) {
            ...
            if (pageCount > 5) {
                throw new IllegalArgumentException("Resume exceeds the maximum limit of 5 pages.");
            }
    ...
    private void recordAtsCheck(boolean hasJobDescription, Map<String, Object> result, String resumeText,
        ...
        if (user != null) {
            ...
            check.setResumeText("Guest session (omitted for privacy)");
        }
