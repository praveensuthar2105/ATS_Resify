...
public class AgentChatService {
    ...
    private void enrichRequestWithPreferences(AgentChatRequest request) {
        try {
            ...
            com.Backend.AI_Resume_Builder_Backend.user.UserPreference pref =
    ...
    private AgentConversation getOrCreateConversation(AgentChatRequest request) {
        if (request.getSessionId() != null && !request.getSessionId().isEmpty()) {
            ...
            if (existing.isPresent()) {
                AgentConversation conv = existing.get();
                if (request.getUserId() != null && !request.getUserId().equals(conv.getUserId())) {
                    throw new org.springframework.web.server.ResponseStatusException(
                            org.springframework.http.HttpStatus.FORBIDDEN, "Access denied to session");
                }
                return conv;
            }
        }
     ...
     */
    public List<AgentMessage> getSessionMessages(String sessionId, String userId) {
        if (userId != null) {
            AgentConversation conv = conversationRepository.findBySessionId(sessionId)
     ...
     */
    @Transactional
    public void endConversation(String sessionId, String userId) {
