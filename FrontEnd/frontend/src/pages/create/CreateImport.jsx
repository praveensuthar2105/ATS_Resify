  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setErrorMessage(result.error || 'Failed to extract structured data from resume.');
      }
    } catch (error) {
      console.error(error);
      setProgressState('error');
      setErrorMessage(error.response?.data?.error || error.message || 'Error occurred during resume parsing.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#f7fdfb] via-[#eefbf7] to-[#d5f5ec] pt-28 pb-16 px-6 relative overflow-hidden flex flex-col items-center justify-start">
      ...
      <div className="max-w-[640px] w-full relative z-10">
        ...
        <div className="glass-panel-tier-1 rounded-3xl p-8 text-center">
          ...
          <h2 className="text-3xl font-bold text-slate-800 font-sans tracking-tight">Import your resume</h2>
          <p className="text-slate-500 text-sm mt-2 max-w-md mx-auto leading-relaxed">
            Upload a PDF or Word file — we'll extract and restructure it for ATS.
          </p>
          ...
          {progressState !== 'success' && (
            <div 
              ...
              className={`mt-8 border-2 rounded-2xl p-10 cursor-pointer transition-all duration-300 flex flex-col items-center justify-center min-h-[240px] group ${
                dragActive 
            ...
            >
              ...
              {/* Gradient chip upload icon - matches site styling */}
              ...
              {selectedFile ? (
                ...
                <div>
                  <p className="text-sm font-bold text-slate-700 m-0">Drag and drop your file here</p>
                  <p className="text-xs text-slate-450 mt-1.5 m-0 font-medium">or click to browse · PDF or DOCX, up to 5MB</p>
                </div>
              )}
            </div>
          )}
          ...
          {progressState !== 'success' && !selectedFile && (
            <div className="flex items-center justify-center gap-4 mt-6 text-xs text-slate-450 font-medium font-sans">
              <span className="flex items-center gap-1 bg-[#14B8A6]/5 border border-[#14B8A6]/10 px-2 py-1 rounded-md">
                <FileText className="w-3 h-3 text-[#0D9488]" /> PDF
              </span>
              <span className="flex items-center gap-1 bg-[#14B8A6]/5 border border-[#14B8A6]/10 px-2 py-1 rounded-md">
                <FileText className="w-3 h-3 text-[#0D9488]" /> DOCX
              </span>
              ...
              <div className="flex flex-col gap-1.5 text-xs text-slate-500">
                ...
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5">
                    ...
                    3. Structuring for ATS (Gemini AI)...
                  </span>
                </div>
              </div>
          ...
          {progressState === 'success' && parsedSummary && (
            <div className="mt-8 p-6 rounded-2xl border border-emerald-105 bg-emerald-50/20 text-center flex flex-col items-center animate-fade-in">
              ...
              <h3 className="text-lg font-bold text-slate-850">Resume parsed successfully!</h3>
              <p className="text-sm text-slate-500 mt-2 max-w-sm">
                Found <strong>{parsedSummary.experience}</strong> experience items · <strong>{parsedSummary.education}</strong> education items · <strong>{parsedSummary.skills}</strong> skills categories.
              </p>
          ...
          {progressState === 'error' && (
            <div className="mt-8 p-5 rounded-2xl border border-red-100 bg-red-50/20 flex flex-col items-center text-center animate-fade-in">
              ...
              <h3 className="text-sm font-bold text-slate-800">Parsing failed</h3>
          ...
          {selectedFile && !loading && progressState !== 'success' && progressState !== 'error' && (
            ...
            >
              Parse my resume <ArrowRight className="w-4 h-4" />
            </button>
          )}
